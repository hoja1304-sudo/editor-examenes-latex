...
- Cuando un plano cartesiano contextualizado use etiquetas direccionales como `$y$ (norte)` y `$x$ (este)`, colocarlas alineadas con la flecha del eje respectivo: `$y$ (norte)` debe quedar centrada sobre el eje vertical y cerca de la punta superior, y `$x$ (este)` debe quedar junto a la punta derecha del eje horizontal. En HTML/SVG, verificar que esas palabras completas no se pierdan ni queden recortadas; si se convierten a texto SVG literal, usar alineacion centrada (`text-anchor="middle"` o equivalente) para `y (norte)` sobre el eje `y` y mantener `x (este)` junto a la flecha del eje `x`.
