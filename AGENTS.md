...
- Las etiquetas que nombran puntos en una grafica (W, H, A, B, C, D, etc.) deben colocarse lo mas cerca posible del punto sin tocarlo, con separacion minima (`inner sep` bajo y, si hace falta, `xshift`/`yshift` de 1 pt). Elegir la posicion (arriba, abajo, izquierda o derecha) que no quede sobre una linea punteada de proyeccion hacia los ejes. Si todas las direcciones tienen punteadas, preferir arriba o abajo con separacion minima.
