...
- Si la rotulacion contextual incluye `$y$ (norte)` y `$x$ (este)`, alinear cada rotulo con la flecha del eje correspondiente: centrar horizontalmente `$y$ (norte)` sobre el eje vertical cerca de la flecha superior y colocar `$x$ (este)` junto a la flecha derecha del eje horizontal. Reservar margen en el bounding box para que ambos rotulos queden completos en PDF y en la conversion SVG/HTML.
