...
- En planos cartesianos con rotulos direccionales `y (norte)` y `x (este)`, verificar que `y (norte)` quede sobre la punta superior de la flecha del eje vertical, centrado con respecto al eje `y`, y que `x (este)` quede cerca de la flecha derecha del eje horizontal. Si el SVG pierde glifos o hay que sustituir rotulos por `<text>`, usar `text-anchor="middle"` o equivalente para centrar `y (norte)` sobre la punta superior del eje `y`, y dejar margen suficiente para que no se recorte.
